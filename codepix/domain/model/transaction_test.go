...
import (
	...
	"github.com/edneyreis999/bank_transfer/domain/model"
...
func TestNewTransaction(t *testing.T) {
	...
	myDescription := "My description mock"
	transaction, err := model.NewTransaction(account, amount, pixKey, myDescription, "")
	//
	require.Nil(t, err)
	...
	require.Equal(t, transaction.Description, myDescription)
	require.Empty(t, transaction.CancelDescription)

	pixKeySameAccount, err := model.NewPixKey(kind, account, key)

	_, err = model.NewTransaction(account, amount, pixKeySameAccount, myDescription, "")
	require.NotNil(t, err)

	_, err = model.NewTransaction(account, 0, pixKey, myDescription, "")
	require.NotNil(t, err)
...
func TestModelChangeStatusOfATransaction(t *testing.T) {
	...
	amount := 3.10
	transaction, _ := model.NewTransaction(account, amount, pixKey, "My description mock", "")
